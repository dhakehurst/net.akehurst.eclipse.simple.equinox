...
public class SimpleArtifactRepositoryFactory extends ArtifactRepositoryFactory {
	...
	File getLocalFile(URI location, IProgressMonitor monitor) throws IOException, ProvisionException {
		File localFile = null;
		URI jarLocation = SimplerArtifactRepository.getActualLocation(location, true);
		URI xmlLocation = SimplerArtifactRepository.getActualLocation(location, false);
		...
		localFile = cache.createCache(location, SimplerArtifactRepository.CONTENT_FILENAME, monitor);
		if (localFile == null) {
		...
		try {
			...
			try {
				...
				if (localFile.getAbsolutePath().endsWith(JAR_EXTENSION)) {
					...
					String entryName = SimplerArtifactRepository.CONTENT_FILENAME + XML_EXTENSION;
				...
				SimplerArtifactRepositoryIO io = new SimplerArtifactRepositoryIO(getAgent());
	...
	public IArtifactRepository create(URI location, String name, String type, Map<String, String> properties) {
		return new SimplerArtifactRepository(getAgent(), name, location, properties);
	}
